...
-- DESCRIPTION: [@FLICKER=1(1=None, 2=Startup, 3=Random, 4=Fixed Interval)]
...
function lightcontrol_properties(e, range_sensing, sensor_range, max_light_distance, speed, mode, light_r, light_g, light_b, flicker, fixed_interval, light_object_name, light_object_no)
	...
	lightcontrol[e].fixed_interval = fixed_interval
...
function lightcontrol_init(e)
	...
	lightcontrol[e].fixed_interval 			= 1	
...
function lightcontrol_main(e)
	...
	if lightcontrol[e].flicker == 4 and lightcontrol[e].mode == 1 then
		if g_Time > rwait[e] and g_Entity[e]['activated'] == 1 then
			SetLightRange(lightNum,current_level[e])
			if lightcontrol[e].light_object_no > 0 then SetEntityEmissiveStrength(lightcontrol[e].light_object_no,current_level[e]) end
