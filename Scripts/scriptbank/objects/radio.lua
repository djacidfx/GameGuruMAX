...
-- DESCRIPTION: [@START_MODE=1(1=Radio Off, 2=Radio On)]
-- DESCRIPTION: [@ACTIVATION=1(1=Manual, 2=Proximity, 3=External)]
-- DESCRIPTION: [@RANDOM_CHANNEL=1(1=Off, 2=On)]
...
function radio_properties(e, use_text, use_range, start_mode, activation, random_channel, prompt_display, item_highlight, highlight_icon_imagefile)
	radio[e].use_text = use_text
	radio[e].use_range = use_range
	radio[e].start_mode = start_mode or 1
	radio[e].activation = activation or 1
	radio[e].random_channel = random_channel or 1
...
function radio_init(e)
	...
	radio[e].start_mode = 1
	radio[e].activation = 1	
	radio[e].random_channel = 1	
...
function radio_main(e)
	...
	if status[e] == "init" then
		...
		if radio[e].start_mode == 2 then SetActivated(e,1) end		
		if radio[e].activation == 3 then radio[e].start_mode = 2 end
		status[e] = "endinit"
	end
	...
	if PlayerDist < radio[e].use_range and radio[e].activation == 1 and radio[e].start_mode == 1 and g_Entity[e]['activated'] == 0 then SetActivated(e,1) end
	
	if PlayerDist < radio[e].use_range and radio[e].activation == 2 and g_Entity[e]['activated'] == 0 then 
		StopSound(e,radio_channel[e])		
		SetActivated(e,1)		
		radio[e].start_mode = 2
	end
	if PlayerDist > radio[e].use_range and radio[e].activation == 2 and g_Entity[e]['activated'] == 1 then
		SetActivated(e,0)
		radio[e].start_mode = 1
	end
	...
	if g_Entity[e]['activated'] == 1 then
		...
		if g_Entity[e]['health'] > 0 then
			if radio[e].start_mode == 2 and shutoff[e] == 0 then
				LoopSound(e,radio_channel[e])
				SetSoundVolume(svolume[e])
			end
			if PlayerDist < radio[e].use_range and radio[e].activation == 1 then
	...
	if g_Entity[e]['activated'] == 0 then
		StopSound(e,radio_channel[e])
		if radio[e].random_channel == 2 then
			radio_channel[e] = math.random(0,2)
		else
			radio_channel[e] = 0
		end		
	end
